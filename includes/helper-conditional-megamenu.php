...
if( !class_exists( 'avia_conditional_mega_menu' ) )
{
    ...
    {
        //stores item ids of all hidden menu items
        ...
        function __construct()
        {
            ...
            add_filter('avia_mega_menu_hide_item', array(&$this,'apply_conditional_rules'), 10, 5);
        }

        function __destruct()
        {
            unset($this->hidden_items);
        }
        ...
        function apply_conditional_rules($show, $output, $item, $depth, $args)
        {
            global $avia_config;
            ...
            if(empty($conditional_logic['enableconditionallogic']) && !empty($this->hidden_items))
            {
                if(in_array($item->menu_item_parent, $this->hidden_items)) $show = false;
            }
            ...
            if(!empty($conditional_logic['enableconditionallogic']) && !empty($avia_config['menu_conditions']) && is_array($avia_config['menu_conditions']))
            {
                ...
                if(function_exists($condition))
                {
                    ...
                    {
                        add_filter('nav_menu_css_class', array(&$this,'apply_css_class'), 10, 2);
                    }
                }
            }
            ...
            if(!$show)
            {
                $this->hidden_items[] = $item->ID;
            }

            return $show;
        }

        function apply_css_class($classes, $item)
        {
            $conditional_logic = $this->get_menu_item_settings($item);

            if(!empty($conditional_logic['conditionalcss'])) $classes[] = $conditional_logic['conditionalcss'];
