...
if(!function_exists('avia_ajax_search'))
{
	...
	function avia_ajax_search()
	{
	    ...
	    if(empty($posts))
	    {
            ...
            $output .= apply_filters('avf_ajax_search_no_criteria_matched', $no_criteria_matched);
	        $output .= "    </span>";
	        $output .= "    <span class='ajax_search_excerpt'>";
	        $another_search_term = __("Please try another search term", 'avia_framework');
            $output .= apply_filters('avf_ajax_search_another_search_term', $another_search_term);
	        $output .= "    </span>";
	        $output .= "</span>";
	        $output .= "</span>";
	        echo $output;
	    ...
	    $query =  apply_filters('avf_ajax_search_view_all_results_link', http_build_query($_REQUEST));
        $label = apply_filters('avf_ajax_search_view_all_results_label', __('View all results','avia_framework'));
	    $output .= "<a class='ajax_search_entry ajax_search_entry_view_all' href='".home_url('?' . $query )."'>".$label."</a>";
    ...
    function avia_sidebar_menu($echo = true)
    {
        ...
        if($subNav && !empty($the_id) && is_page())
        {
            ...
            if ($children)
            {
                ...
                $sidebar_menu .= "<div class='widget widget_nav_menu $display_child_pages'><ul class='nested_nav'>";
