		...
		private function default_slide()
		{
			...
			foreach($this->id_array as $id)
			{
				if(isset($this->slides[$id]))
				{
					...
					$html .= "<".$tags[0]." >{$caption}<img src='".$img[0]."' title='".$imgtitle."' alt='".$imgalt."' /></ ".$tags[1]." >";
					$html .= "</li>";
				}
				else
				{
		...
		private function advanced_slide()
		{
			...
			foreach($this->id_array as $id)
			{
				if(isset($this->slides[$id]))
				{
					...
					if(trim($content) != "") $content 	= "<div class='avia-caption-content'>".ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop(trim($content)))."</div>";
