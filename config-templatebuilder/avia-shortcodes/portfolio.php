...
if ( !class_exists( 'avia_post_grid' ) )
{
	class avia_post_grid
	{
		...
		private function sort_buttons($entries, $params)
		{
			...
			$output .= "<a href='#' data-filter='all_sort' class='all_sort_button active_sort'><span>".$first_item_name."</span><small class='avia-cat-count'> ".count($entries)." </small></a>";
			...
			foreach($categories as $category)
			{
				if(in_array($category->term_id, $current_page_cats, true))
				{
					...
					$output .= "<a href='#' data-filter='".$category->category_nicename."_sort' class='".$category->category_nicename."_sort_button' >
						<span>".$category->cat_name."</span>
						<small class='avia-cat-count'> ".$cat_count[$category->term_id]." </small>
					</a>";
				}
			}
