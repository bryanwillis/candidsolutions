...
if ( !class_exists( 'aviaShortcodeTemplate' ) ) {
	...
	abstract class aviaShortcodeTemplate
	{
		...
		*	$this->config['name'] = __('Text', 'avia_framework' ); //defines the name of the button that is displayed bellow the icon
		...
		*/
		private function extra_config_element_iterator($elements)
		{
		...
		*/
		private function create_asset_array()
		{
			if(!empty($this->elements))
			{
				foreach ($this->elements as $element)
				{
					...
					{
						AviaBuilder::$resources_to_load[$element['font']] = $element;
					}
				}
			}
		}
		...
		*/
		...
		function class_by_arguments($classNames, $args, $classNamesOnly = false)
		{
			...
			{
				$classes .= "avia-$class-".str_replace(" ","_",$args[$class])." ";
			}
